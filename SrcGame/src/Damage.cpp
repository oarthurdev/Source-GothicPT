...
int dm_SendTransDamage(smCHAR *lpChar, int PowParam1, int PowParam2, int AttackState, int Resistance, DWORD dwSkillCode, int UseAccuracy, smWINSOCK *lpsmSock)
{
	...
	// Dano adicional skill adagas da assasina aqui
	if (lpCurPlayer->smCharInfo.JOB_CODE == JOBCODE_ASSASSINE)
	{
		if(((lpCurPlayer->dwActionItemCode ^ sinWD1) & sinITEM_MASK2) == 0)
		{
			extern int Skill_DMastery_Level;
			if (Skill_DMastery_Level > 0)
			{
				TransAttackData.Power[0] += (TransAttackData.Power[0] * D_Mastery_DamagePercent[Skill_DMastery_Level]) / 100;
				TransAttackData.Power[1] += (TransAttackData.Power[1] * D_Mastery_DamagePercent[Skill_DMastery_Level]) / 100;
			}
		}
	}
