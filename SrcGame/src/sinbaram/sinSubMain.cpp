...
void InitCursor()
{
	MatCursorDefault = CreateTextureMaterial("game\\GUI\\Cursor\\DefaultCursor.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorSell = CreateTextureMaterial("game\\GUI\\Cursor\\SellCursor.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorBuy = CreateTextureMaterial("game\\GUI\\Cursor\\BuyCursor.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorRepair = CreateTextureMaterial("game\\GUI\\Cursor\\RepairCursor.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorAttack = CreateTextureMaterial("game\\GUI\\Cursor\\Attack_Cursor.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorGetItem1 = CreateTextureMaterial("game\\GUI\\Cursor\\GetItem_Cursor1.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorGetItem2 = CreateTextureMaterial("game\\GUI\\Cursor\\GetItem_Cursor2.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatCursorTalk = CreateTextureMaterial("game\\GUI\\Cursor\\Talk_Cursor.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
}
...
int sinSetInvenItem(sITEMINFO *sItemInfo)
{
	...
	if (LoadItemImage(sItemInfo, &sinTempItem))
	{
		...
		else
		{
			...
			if (Shop::GetInstance()->IsOpen() || TShop::GetInstance()->IsOpen())
			{
				if (cInvenTory.CheckRequireItemToSet(&sinTempItem))
				{
					if (!cInvenTory.AutoSetInvenItem(&sinTempItem))
...
int StopCharMotion(int x, int y)
{
	...
	if (Shop::GetInstance()->IsOpen() || TShop::GetInstance()->IsOpen() || cSettings::GetInstance()->IsOpen() || Battle::GetInstance()->IsOpen())
		return TRUE;
	...
	if(smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
	{
		if(sinSecretFlag)
		{
			if(CheatHelpOpenFlag)
			{
				...
				dsTextLineOut( 100, 200, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(L) 돈 200000 ");
				dsTextLineOut( 100, 230, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(B) LV 65 돈 300000");
				dsTextLineOut( 100, 260, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(Y) 믹스쳐 아이템 실행 ");
				dsTextLineOut( 100, 290, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(U) 전업  ");
				dsTextLineOut( 100, 320, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(F) 숙련도 Up  ");
				dsTextLineOut( 100, 350, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(N) 숙련도 Down  ");
				dsTextLineOut( 100, 380, strBuff, lstrlen(strBuff));
			}
		}
	}
...
void sinTestKey()
{
	...
	if (smConfig.getSecurity() >= AccountTypes::SEC_GAMEMASTER)
	{
		// Mature Automatico
...
int NumLineComa64(INT64 Num, char* pBuff)
{

	int i = 0;
	char strBuff[128];
	wsprintf(strBuff, "%I64i", Num);
	int cnt = 0;
	int cnt2 = 0;
	int cnt3 = 0;
