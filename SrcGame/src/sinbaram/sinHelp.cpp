...
void cSINHELP::Main()
{
	...
	for (int i = 0; i < SIN_MAX_HELP_NUMBER; i++)
	{
		if (sSinHelp[i].KindFlag)
		{
			...
			{
				if (chaPremiumitem.TelePortCore.PageIndex == TELEPORT_PAGEMAX)
					Temp = TELEPORT_PAGEULTM_CORE;
				else
					Temp = TELEPORT_PAGES_CORE;
...
void cSINHELP::LButtonDown(int x, int y)
{
	...
	for (int i = 0; i < SIN_MAX_HELP_NUMBER; i++)
	{
		if (sSinHelp[i].KindFlag)
		{
			...
			if (sSinHelp[i].KindFlag == SIN_HELP_KIND_BATTLE_ARENA)
			{
			...
			if (sSinHelp[i].KindFlag == SIN_HELP_KIND_QUEST_DIARY)
			{
				if (QuestDiaryState == 0)
				{
					cMessageBox.ShowMessage3(MESSAGE_QUEST_DIARY, "Deseja comeÁar?");
				}
				else
					if (QuestDiaryState == 1 && QuestDiaryKill1 == QuestDiaryKill2)
					{
						cMessageBox.ShowMessage3(MESSAGE_QUEST_DIARY, "Deseja finalizar?");
					}
			}
...
void cSINHELP::DrawText()
{
	...
	for (i = 0; i < SIN_MAX_HELP_NUMBER; i++)
	{
		if (sSinHelp[i].KindFlag)
		{
			...
			if (sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_SCROLL)
			{
				for (j = 0; j < tHeight - 1; j++)
				{
					if (sSinHelp[i].szDocBuff[j + TempLine4])
					{
						...
						dsTextLineOut( sSinHelp[i].PosiX + 30, sSinHelp[i].PosiY + 66 + (15 * j), strBuff, lstrlen(strBuff));
					}
					else
					{
						sinTest5 = 1;
			...
			else if ((sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_MILTER) || (sSinHelp[i].KindFlag == SIN_HELP_KIND_TRADEBPEXP))
			{
				for (j = 0; j < tHeight - 1; j++) {
					if (sSinHelp[i].szDocBuff[j + TempLine4]) {
						wsprintf(strBuff, "%s", sSinHelp[i].szDocBuff[j + TempLine4]);
			...
			if (sSinHelp[i].KindFlag == SIN_HELP_KIND_NORMAL)
			{
				...
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, HelpTitle8, lstrlen(HelpTitle8));
			...
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_SOD)
			{
				for (int h = 0; h < 10; h++)
				{
					if (sSod_Rank[h].Flag)
					{
						...
						if (h == 9)
						{
							dsTextLineOut( sSinHelp[i].PosiX + 20, sSinHelp[i].PosiY + 64 + (h * 24), strBuff, lstrlen(strBuff));
						}
						else
						{
							dsTextLineOut( sSinHelp[i].PosiX + 25, sSinHelp[i].PosiY + 64 + (h * 24), strBuff, lstrlen(strBuff));
						}
						//SelectObject(hdc, sinFont);
						
						SetFontTextColor( RGB(222, 231, 255));
						Len = lstrlen(sSod_Rank[h].Name);
						dsTextLineOut( sSinHelp[i].PosiX + 25 + (116 - ((Len * 6) / 2)), sSinHelp[i].PosiY + 64 + (h * 24), sSod_Rank[h].Name, lstrlen(sSod_Rank[h].Name));
						...
						Len = lstrlen(strBuff);
						dsTextLineOut( sSinHelp[i].PosiX + 25 + 97 + (116 - ((Len * 6) / 2)), sSinHelp[i].PosiY + 64 + (h * 24), strBuff, lstrlen(strBuff));
						...
						Len = lstrlen(strBuff);
						dsTextLineOut( sSinHelp[i].PosiX + 25 + 97 + 70 + (116 - ((Len * 6) / 2)), sSinHelp[i].PosiY + 64 + (h * 24), strBuff, lstrlen(strBuff));
					}
				}
			...
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_C_TELEPORT)
			{
				//SelectObject(hdc, sinBoldFont);
				SetFontTextColor( RGB(255, 255, 240));
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, HaWarpGateName[0], lstrlen(HaWarpGateName[0]));
			}
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_WARPGATE)
			{
				...
				if (sSinWarpGate.GateSelect > -1)
					dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, WarpGateName[sSinWarpGate.GateSelect + 1], lstrlen(WarpGateName[sSinWarpGate.GateSelect + 1]));
				else
					dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, WarpGateName[0], lstrlen(WarpGateName[0]));
			}
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_INITPOINT)
			{
				//SelectObject(hdc, sinBoldFont);
				SetFontTextColor( RGB(255, 255, 240));
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, sinWarningName5, lstrlen(sinWarningName5));
			...
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_BABEL || (sSinHelp[i].KindFlag == SIN_HELP_KIND_TRADEBPEXP))
			{
				//SelectObject(hdc, sinBoldFont);
				SetFontTextColor( RGB(255, 255, 240));
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, QuestTitle8, lstrlen(QuestTitle8));
			...
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_RESEARCH)
			{
				//SelectObject(hdc, sinBoldFont);
				SetFontTextColor( RGB(255, 255, 240));
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, ResearchTitle8, lstrlen(ResearchTitle8));
			}
			else if (sSinHelp[i].KindFlag == SIN_HELP_KIND_TELEPORT_SCROLL)
			{
				...
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 17, sSinHelp[i].TitlePosi.y + 4, TeleportTitle8, lstrlen(TeleportTitle8));
#else
				dsTextLineOut( sSinHelp[i].TitlePosi.x + 35, sSinHelp[i].TitlePosi.y + 9, TeleportTitle8, lstrlen(TeleportTitle8));
#endif
			}
...
int cSINHELP::TonyGetFileData()
{
	...
	while (!feof(fp))
	{
		...
		if (!NextFlag && lstrcmp(sinDecode, "*¿ÃπÃ¡ˆ∆ƒ¿œ¿Ã∏ß") == 0)
		{
			p = GetWord(strBuff, p);
		...
		if (!NextFlag && lstrcmp(sinDecode, "*¿ÃπÃ¡ˆ¿ßƒ°") == 0)
		{
			p = GetWord(strBuff, p);
		...
		if (!NextFlag && lstrcmp(sinDecode, "*¿ÃπÃ¡ˆ≈©±‚") == 0)
		{
			p = GetWord(strBuff, p);
		...
		if (!NextFlag && lstrcmp(sinDecode, "*∆ƒ¿œ∆˜∏À") == 0)
		{
			p = GetWord(strBuff, p);
