...
sINVENTORY sInven[INVENTORY_MAX_POS] = {
	...
	{ INVENTORY_POS_EARRINGSL  ,{ 506 + 1 , 488 + 1 , 510 + 22 , 488 + 22 } },
	{ INVENTORY_POS_EARRINGSR  ,{ 532 + 1 , 488 + 1 , 536 + 22 , 488 + 22 } },
};
...
int MagicForceOrbUseLevel[][2] = {
	{ 1, 19 }, { 1, 29 }, { 1, 39 }, { 1, 49 }, { 50, 200 }, { 50, 200 }, { 50, 200 }, { 55, 200 }, { 60, 200 }, { 65, 200 }, { 70, 200 }, { 90, 200 }, { 10, 200 }, { 10,200 },
};
...
// Limite de uso de cristais
#define MAX_USE_CRISTAL 8
...
void cINVENTORY::RButtonDown(int x, int y)
{
	...
	else if (cShop.OpenFlag)
	{
		if (SelectInvenItemIndex)
		{
			...
			if ((Item.sItemInfo.CODE == (sinCA2 | sin13) || Item.sItemInfo.CODE == (sinCA2 | sin14))) return;
	...
	{
	// Botao direito trade aqui
		cTrade.SendItemToIven(x, y, 1);
		if (SelectInvenItemIndex)
		{
			sITEM & Item = InvenItem[SelectInvenItemIndex - 1];
			
			// Trajes no trade aqui
			if ((Item.sItemInfo.CODE == (sinCA2 | sin13) || Item.sItemInfo.CODE == (sinCA2 | sin14))) return;
	...
	if (SelectInvenItemIndex)
	{
		...
		if ((InvenItem[SelectInvenItemIndex - 1].CODE &sinITEM_MASK2) == sinGP1)
		{
			...
			// Desativado: Delay dos Cristais
			/*
			if (DelayTime && (GetCurrentTime() - DelayTime < 20000)) { 

				if (chaPremiumitem.haItemUseDelayFlag)
				{
					cMessageBox.ShowMessage("Aguarde 20 segundos para usar um novo cristal");
					return;
				}
			...
			if (nCristalInUse >= MAX_USE_CRISTAL) {
				cMessageBox.ShowMessage("Você só pode utilizar 8 cristais por vez!");
				return;
			}
			else {
				nCristalInUse++;
		...
		if ((InvenItem[SelectInvenItemIndex - 1].CODE &sinITEM_MASK2) == sinBI1 || (InvenItem[SelectInvenItemIndex - 1].CODE &sinITEM_MASK2) == sinBI2)
		{
			...
			if (InvenItem[SelectInvenItemIndex - 1].sItemInfo.CODE == (sinBI2 | sin85))
			...
			if (InvenItem[SelectInvenItemIndex - 1].sItemInfo.CODE == (sinBI2 | sin84))
			...
			if (InvenItem[SelectInvenItemIndex - 1].sItemInfo.CODE == (sinBI2 | sin95))
				chaPremiumitem.UsePremiumItem(166);

			if (InvenItem[SelectInvenItemIndex - 1].sItemInfo.CODE == (sinBI2 | sin96))
				chaPremiumitem.UsePremiumItem(167);


			if (InvenItem[SelectInvenItemIndex - 1].sItemInfo.CODE == (sinBI2 | sin97))
				chaPremiumitem.UsePremiumItem(168);
				...
				InvenItem[i].y < y  && InvenItem[i].y + InvenItem[i].h > y)
			{
				...
				if (PickUpFlag == 1)
				{
					...
					if (CursorClass == SIN_CURSOR_SELL)
					{
						...
						// Mudança aqui
						if (InvenItem[i].sItemInfo.CODE == (sinCA2 | sin13) || InvenItem[i].sItemInfo.CODE == (sinCA2 | sin14))
						{
							return TRUE;
						}
...
int cINVENTORY::NoSettingThrowItem(sITEM *pItem, int Flag)
{
	...
	// Mudança aqui
	if (pItem->sItemInfo.CODE == (sinCA2 | sin13) || pItem->sItemInfo.CODE == (sinCA2 | sin14))
	{
		return FALSE;
	}
...
int cINVENTORY::CharOnlySetItem(sITEM *pItem)
{
	...
	{
		if ((pItem->sItemInfo.CODE & sinITEM_MASK2) == sinDS1) 
		{
			pItem->sItemInfo.NotUseFlag = 1;
		}
	}
...
int cINVENTORY::DrawInvenText()
{
	...
	if (OpenFlag)
	{
		...
		dsTextLineOut( CheckEditSize(InvenTextPosi[0][0] + 26, InvenTextPosi[0][2], strBuff), InvenTextPosi[0][1] + (250 - sinMoveKindInter[SIN_INVENTORY]),
			strBuff, lstrlen(strBuff));

		wsprintf(strBuff, "%s coins", Shop::GetInstance()->coinsInvetario());
		dsTextLineOut(379, 500, strBuff, lstrlen(strBuff));
		//cShop.coinsInvetario();
		/*
		NumLineComa(cShop.Coin, sCoin);
		wsprintf(strBuff, "Coins %s", sCoin);
		dsTextLineOut(379, 500, strBuff, lstrlen(strBuff));
		*/

		wsprintf(strBuff, "%d/%d", sinChar->Weight[0], sinChar->Weight[1]);
		dsTextLineOut( CheckEditSize(InvenTextPosi[1][0], InvenTextPosi[1][2], strBuff), InvenTextPosi[1][1] + (250 - sinMoveKindInter[SIN_INVENTORY]),
			strBuff, lstrlen(strBuff));
	}
		...
		if (InvenItem[i].Flag)
		{
			...
			if (!InvenItem[i].lpItem)
			{
				if (InvenItem[i].x > 10)
				{
					...
					strcpy_s(strBuff, "NO");
					dsTextLineOut( NoImageMessagePosi.x, NoImageMessagePosi.y + (250 - sinMoveKindInter[SIN_INVENTORY]), strBuff, lstrlen(strBuff));
					...
					dsTextLineOut( NoImageMessagePosi.x, NoImageMessagePosi.y + (250 - sinMoveKindInter[SIN_INVENTORY]) + 20, strBuff, lstrlen(strBuff));
				}
			}
			if (InvenItem[i].Class == ITEM_CLASS_POTION)
			{
				...
				if (InvenItem[i].ItemPosition == 11 || InvenItem[i].ItemPosition == 12 || InvenItem[i].ItemPosition == 13)
				{
					dsTextLineOut( InvenItem[i].x, InvenItem[i].y, strBuff, lstrlen(strBuff));
				}
				else
				{
					if (OpenFlag)
						dsTextLineOut( InvenItem[i].x, InvenItem[i].y + (250 - sinMoveKindInter[SIN_INVENTORY]) + 5, strBuff, lstrlen(strBuff));
				}
			}
		}
	}
	...
	if (nName == 1)
	{
		...
		dsTextLineOut( BackStartPos.x + 60, BackStartPos.y, "Name", lstrlen("Name"));
		...
		for (int i = 0; i < len; i++)
		{
			...
			dsTextLineOut( BackStartPos.x + 32, BackStartPos.y + (i * 20) - 110, szBuff2, lstrlen(szBuff2));
...
int cINVENTORY::SetItemToChar(int CheckHackFlag)
{
	...
	//Set Boss #1
	if (cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.CODE == (sinOR1 | sin25)) {
		LValRing = 1;
	}
	else { LValRing = 0; }
	if (cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.CODE == (sinOR1 | sin25)) {
		RValRing = 1;
	}
	else { RValRing = 0; }
	if (cInvenTory.InvenItem[sInven[3].ItemIndex - 1].sItemInfo.CODE == (sinOA1 | sin25)) {
		KelAmulet = 1;
	}
	else { KelAmulet = 0; }

	//Set Boss #2
	if (cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.CODE == (sinOR1 | sin32)) {
		LShyRing = 1;
	}
	else { LShyRing = 0; }
	if (cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.CODE == (sinOR1 | sin32)) {
		RShyRing = 1;
	}
	else { RShyRing = 0; }
	if (cInvenTory.InvenItem[sInven[3].ItemIndex - 1].sItemInfo.CODE == (sinOA1 | sin32)) {
		TullaAmulet = 1;
	}
	else { TullaAmulet = 0; }
	...
	for (int i = 0; i < 10; i++)
	{
		if (ContinueSkill[i].Flag)
		{
			...
			case SOD_SKILL_SILVER:
				sinAdd_Defence += (10 * sinChar->Defence) / 100;
				break;

			case SOD_SKILL_BRONZE:
				sinAttack_Rating += (5 * sinChar->Attack_Rating) / 100;
				break;
			...
			case PLAYERGM:
				//sinTempDamage2[0] += (int)(sinChar->Attack_Damage[0] * 50) / 100;
				//sinTempDamage2[1] += (int)(sinChar->Attack_Damage[1] * 50) / 100;
				//sinfIncreLife += 1000.0f;
				//sinfIncreMana += 1000.0f;
				//sinfIncreStamina += 1000.0f;
				//extern int SetSODSkill(int SODSkill);
				//SetSODSkill(1);
				SkillTextColor[SIN2_DAMAGE] = SIN_TEXT_COLOR_GOLD;
				SkillTextColor[SIN2_LIFE] = SIN_TEXT_COLOR_GOLD;
				break;
			...
			case BATTLE_LEVEL:
				int szIncreLife; int szIncreMana; int IncreStamina;
				szIncreLife = 0; szIncreMana = 0; IncreStamina = 0;

				switch (CharBattleLevel)
				{
				case 1:
					szIncreLife = 50;
					szIncreMana = 50;
				...
				sinfIncreLife += szIncreLife;
				sinfIncreMana += szIncreMana;
				sinfIncreStamina += IncreStamina;
	...
	//DEFESA DO CHAR
	sinChar->Defence = (int)((fDexterity / 2) + (ftalent / 3) + fLevel * 1.4 + sinDefense) + sinAdd_Defence;
	fDefense = (float)sinChar->Defence;

	/*sinChar->Defence = (int)((fDexterity / 6) + (ftalent / 4) + (fSpirit / 6) + fLevel * 1.3 + sinDefense) + sinAdd_Defence;
	fDefense = (float)sinChar->Defence;*/ //ANTES DO BALANCEAMENTO
	...
	/*CountAbsorbItem /= 2;
	if (CountAbsorbItem > 2) CountAbsorbItem = 2;
	sinChar->Absorption = (int)((fDefense / 100 + (fLevel / 10) + sinAbsorption + sinAdd_fAbsorb) + ((fstrength + ftalent + fSpirit) / 40)) + 1 + CountAbsorbItem;*/ //ANTES DO BALANCEAMENTO
	...
	int baseWeight = int((((fstrength * 2.0f) + (fhealth * 1.4f) + fLevel * 3.0f) + 60 + fDexterity * 1.2f + fSpirit * 2.0f));
	/*int baseWeight = int((((fstrength * 2.0f) + (fhealth * 1.4f) + fLevel * 3.0f) + 60 + fDexterity * 1.2f + fSpirit * 0.9f));*/ //ANTES BALANCO

	if (chaPremiumitem.m_MightofAwellTime > 0)
	{
		sinChar->Weight[1] = (short)(baseWeight + chaPremiumitem.m_MightofAwellWeight);
	}
	else
	{
		sinChar->Weight[1] = (short)baseWeight;
	}
	...
	{
		sinChar->Weight[1] = (short)(baseWeight + (sinChar->GravityScroolCheck[0]) * 50);
	...
	case 1:
		...
		break;
	case 2:
		sinChar->Life[1] = (short)(fLevel * 1.8 + ((fhealth * 1.6) + (fSpirit * 1.6)) - 5);             //XAMA
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 3:
		...
		break;
	case 4:
		sinChar->Life[1] = (short)(fLevel * 1.7 + ((fhealth * 1.6) + (fSpirit * 1.6)));               //SACER
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 5:
		sinChar->Life[1] = (short)(fLevel * 1.5 + ((fhealth * 1.3) + (fSpirit * 1.3)));                //MAGO
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	...
	{
	case 1:
		sinChar->Life[1] = (short)(fLevel * 2.1 + ((fhealth * 2.5) + (fstrength * 0.9)) - 10);  // LUTADOR
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 2:
		sinChar->Life[1] = (short)(fLevel * 2.1 + ((fhealth * 2.3) + (fstrength * 0.9)) - 5);   ///MECANICO
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 3:
		sinChar->Life[1] = (short)(fLevel * 1.8 + ((fhealth * 6.0) + (fDexterity * 0.9)) - 5);   //ARCHER
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 4:
		sinChar->Life[1] = (short)((fLevel * 2.1) + (fhealth * 2.5) + (fstrength * 1.0)) - 5;   ///PIKE
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 5:
		sinChar->Life[1] = (short)(fLevel * 1.8 + ((fhealth * 7.0) + (fDexterity * 0.9)) - 5); ///ATALANTA
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 6:
		sinChar->Life[1] = (short)(fLevel * 1.9 + ((fhealth * 2.5) + (fstrength * 0.9)) - 5);    /// KINA
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 7:
		sinChar->Life[1] = (short)(fLevel * 2.1 + ((fhealth * 2.5) + (fSpirit * 0.9)) - 5);  ///MAGO
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 8:
		sinChar->Life[1] = (short)(fLevel * 2.0 + ((fhealth * 1.8) + (fSpirit * 0.8)) - 5);  ///SACER
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 9:
		sinChar->Life[1] = (short)((fLevel * 2.1) + (fhealth * 2.5) + (fstrength * 1.0) - 5); ///ASSASSINA
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	case 10:
		sinChar->Life[1] = (short)(fLevel * 2.1 + ((fhealth * 2.5) + (fSpirit * 0.9)) - 5); ///XAMA
		sinChar->Life[1] += (short)(sinLev_Life + sinfIncreLife);
		break;
	...
	{
	case 1:
		sinChar->Mana[1] = (short)((fLevel * 0.9f + (fSpirit*2.3f)));         //MANA MAGO
	...
	case 2:
		...
		break;
	case 3:
		sinChar->Mana[1] = (short)(sinChar->Level * .6f + (sinChar->Spirit*2.2f));
		sinChar->Mana[1] += (short)(sinLev_Mana + sinfIncreMana);
		break;
	...
	for (int i = 1; i < MAX_USESKILL; i++)
	{
		if (sinSkill.UseSkill[i].Flag && sinSkill.UseSkill[i].Point)
		{
			...
			case SKILL_BOOST_HEALTH:
				...
				break;
			case SKILL_L_PEACE:
				TempSkillData = (float)((float)(InnerPeace_Add[sinSkill.UseSkill[i].Point - 1]) * ((float)sinChar->Mana[1] / 100.0f));
				sinChar->Mana[1] += (short)TempSkillData;
				break;
			case SKILL_D_MASTERY:
				extern int Skill_DMastery_Level;
				Skill_DMastery_Level = sinSkill.UseSkill[i].Point - 1;
				break;
			case SKILL_A_MASTERY:
				extern int Skill_AMastery_Evade;
				Skill_AMastery_Evade = A_Mastery_AddPercent[sinSkill.UseSkill[i].Point - 1] / 10;
				break;
			case SKILL_F_MASTERY:
				if(sinChar->Critical_Hit > 0) // < Evita divisão por 0
				sinChar->Critical_Hit += (int)(sinChar->Critical_Hit * F_Mastery_Critical[sinSkill.UseSkill[i].Point - 1]) / 100;
				break;
			}
			...
			case SKILL_A_MIGAL:
				...
				for (p = 0; p < 8; p++)
				{
					if (sInven[0].ItemIndex)
					{
						if (ContinueSkill[i].Skill_Info.UseWeaponCode[p] == (cInvenTory.InvenItem[sInven[0].ItemIndex - 1].CODE & sinITEM_MASK2))
						{
							sinChar->Attack_Damage[0] += AdventMigal_AttackRate[ContinueSkill[i].Point - 1] / 2;
							sinChar->Attack_Damage[1] += AdventMigal_AttackRate[ContinueSkill[i].Point - 1] / 2;
...
int cINVENTORY::UseEcore(sITEM *pItem)
{
	...
	// Core de Party
	if (pItem->sItemInfo.CODE == (sinEC1 | sin06) || pItem->sItemInfo.CODE == (sinEC1 | sin05)) {

		if (pItem->sItemInfo.CODE == (sinEC1 | sin06)) {
			...
			if (pItem->sItemInfo.ItemName[len + 1] == NULL) {
				...
				if (RequestPartyPlayers(lpCurPlayer)) {
					lstrcpy(szLinkName, &pItem->sItemInfo.ItemName[len + 1]);
					for (int cnt = 0; cnt < cParty.MemberCount; cnt++) {
						if (stricmp(cParty.Members[cnt].szName, szLinkName) == 0) {
					...
					cMessageBox.ShowMessage("Você deve estar em grupo");
				}

				if (!RequestPartyPlayers(lpCurPlayer) && !foundPlayerGroup) AddChatBuff("> Membro do grupo não encontrado!");
			}
		}
