...
smOBJ3D::~smOBJ3D()
{
	...
	if(TmScale) delete[TmScaleCnt] TmScale;
	if(TmPos) delete[TmPosCnt] TmPos;
	if(TmRot) delete[TmRotCnt] TmRot;
	if(TmPrevRot) delete[TmRotCnt] TmPrevRot;

	if(TexLink) delete[nTexLink] TexLink;
	if(Physique) delete[] Physique;
	if(Face) delete[nFace] Face;
	if(Vertex) delete[nVertex] Vertex;
