...
static  char					*s_Item2PassTexName[MAX_ITEM2PASS_TEX] =
{
	...
	"mixS_06.bmp", "mixS_07.bmp", "mixS_08.bmp", "mixS_09.bmp", "mixS_10.bmp",
	"mixM_01.bmp", "mixM_02.bmp", "mixM_03.bmp", "mixM_04.bmp", "mixM_05.bmp",
	"mixM_06.bmp", "mixM_07.bmp", "mixM_08.bmp", "mixM_09.bmp", "mixM_10.bmp",
};
...
int smRENDER3D::SetItem2PassD3DRendState(smMATERIAL *lpMaterial)
{
	...
	if (lpMaterial->TextureType == SMTEX_TYPE_MULTIMIX)
	{
		...
		for (cnt = 0; cnt < 2; cnt++)
		{
			...
			d3ddev->SetTextureStageState(cnt, D3DTSS_TEXCOORDINDEX, cnt);

			if (cnt == 0)
				d3ddev->SetTextureStageState(cnt, D3DTSS_COLOROP, D3DTOP_MODULATE);
			else 
				d3ddev->SetTextureStageState(cnt, D3DTSS_COLOROP, D3DTOP_ADD);
			...
			if (cnt == 0)
				d3ddev->SetTexture(cnt, lpMaterial->smTexture[cnt]->lpD3DTexture);
			else 
				d3ddev->SetTexture(cnt, s_lpItem2PassTexHandle->lpD3DTexture);
		...
		for (cnt = 2; cnt < 8; cnt++)
		{
			d3ddev->SetTextureStageState(cnt, D3DTSS_TEXCOORDINDEX, 0);
	...
	if (MapOpacity) {
		...
		d3ddev->SetRenderState(D3DRS_ALPHAREF, AlphaTestDepth);
...
int smRENDER3D::SetD3DRendState(smMATERIAL *lpMaterial, int TexClip)
{
	...
	case SMTEX_TYPE_MULTIMIX:
		for (cnt = 0; cnt < lpMaterial->TextureCounter; cnt++) {
			...
			if (lpMaterial->TextureStageState[cnt])
				d3ddev->SetTextureStageState(cnt, D3DTSS_COLOROP, lpMaterial->TextureStageState[cnt]);
			else
				d3ddev->SetTextureStageState(cnt, D3DTSS_COLOROP, D3DTOP_MODULATE);
			...
			//if (TexClip == TRUE && lpMaterial->smTexture[cnt]->lpD3DTextureLarge)
				//d3ddev->SetTexture(cnt, lpMaterial->smTexture[cnt]->lpD3DTextureLarge);
			//else
				d3ddev->SetTexture(cnt, lpMaterial->smTexture[cnt]->lpD3DTexture);
	...
	case SMTEX_TYPE_ANIMATION:
		...
		d3ddev->SetTexture(0, lpMaterial->smAnimTexture[cnt]->lpD3DTexture);
...
int smRENDER3D::SetD3DRendStateOnlyAlpha(smMATERIAL *lpMaterial, int TexClip)
{
	...
	for (cnt = 0; cnt < lpMaterial->TextureCounter; cnt++)
	{
		...
		if (lpMaterial->smTexture[cnt]->MapOpacity)
		{
			...
			d3ddev->SetTextureStageState(MapCnt, D3DTSS_TEXCOORDINDEX, cnt);

			d3ddev->SetTextureStageState(MapCnt, D3DTSS_COLOROP, D3DTOP_MODULATE);
			d3ddev->SetTextureStageState(MapCnt, D3DTSS_COLORARG1, D3DTA_TEXTURE);
			d3ddev->SetTextureStageState(MapCnt, D3DTSS_COLORARG2, D3DTA_CURRENT);

			d3ddev->SetTextureStageState(MapCnt, D3DTSS_ALPHAOP, D3DTOP_MODULATE);
			d3ddev->SetTextureStageState(MapCnt, D3DTSS_ALPHAARG2, D3DTA_CURRENT);
			d3ddev->SetTextureStageState(MapCnt, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);

			//if (TexClip == TRUE && lpMaterial->smTexture[cnt]->lpD3DTextureLarge)
				//d3ddev->SetTexture(MapCnt, lpMaterial->smTexture[cnt]->lpD3DTextureLarge);
			//else
				d3ddev->SetTexture(MapCnt, lpMaterial->smTexture[cnt]->lpD3DTexture);
...
int smRENDER3D::RenderD3D(int MatNum)
{
	...
	if (RendMatrial[MatNum].MatrialCounter)
	{
		...
		d3ddev->SetFVF(D3DFVF_TEXTURE[smRendMaterial[MatNum].TextureCounter + TexCnt]);
		if ((rendcnt / 3) > 0)
			d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, rendcnt / 3, (LPVOID)RendBuff, D3DFVF_SIZE[smRendMaterial[MatNum].TextureCounter + TexCnt]);
	...
	if (RendMatrial[MatNum].TexClip_MatrialCounter)
	{
		...
		d3ddev->SetFVF(D3DFVF_TEXTURE[smRendMaterial[MatNum].TextureCounter]);
		if ((rendcnt / 3) > 0)
			d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, rendcnt / 3, (LPVOID)RendBuff, D3DFVF_SIZE[smRendMaterial[MatNum].TextureCounter]);
...
int smRENDER3D::RenderD3D_OutLine(void)
{
	...
	d3ddev->SetRenderState(D3DRS_TEXTUREFACTOR, OutLineColor);
	...
	for (cnt = 0; cnt < RMatListCnt; cnt++)
	{
		...
		{
			d3ddev->SetTexture(0, lpMaterial->smTexture[0]->lpD3DTexture);
...
int smRENDER3D::RenderD3D_OutLine(int MatNum)
{
	...
	if ((rendcnt / 3) > 0)
		d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, rendcnt / 3, (LPVOID)RendBuff, 0x1C);
