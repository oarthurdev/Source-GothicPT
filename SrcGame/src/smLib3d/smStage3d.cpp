...
int smSTAGE3D::Close()
{
	...
	if(nLight) delete[nLight] smLight;

	if(lpwAreaBuff) delete[wAreaSize] lpwAreaBuff;

	if(nTexLink) delete[nTexLink]	TexLink;
	if(nFace)	delete[nFace]	Face;
	if(nVertex)	delete[nVertex]	Vertex;
...
{
	if(!Head) //FAZ A TELA DE LOGUIN APARECER
		return 0;
...
int	smSTAGE3D::LoadFile(char *szFile)
{
	...
	if(!hFile)
	{
		Log::getInstance()->output(LOG_ERROR, "Cannot open StageFile(%s)", szFile);
		return FALSE;
	}
