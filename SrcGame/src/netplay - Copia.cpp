...
#else
#define	CLIENT_VERSION_NUM		10000
#endif
#endif
...
smWINSOCK *ConnectServer_Main()
{
	...
	strcpy_s(server2, sizeof(server2), Settings::getInstance()->getServer2().c_str());
	strcpy_s(server3, sizeof(server3), Settings::getInstance()->getServer3().c_str());
	...
	if (!smWsockServer)
	{
		...
		if (Settings::getInstance()->getPort())
			dwServPort = Settings::getInstance()->getPort();
		else
			dwServPort = 10009;
	...
	{
		const char* server1 = Settings::getInstance()->getServer1().c_str();

		if (server1[0] && lstrcmpi(server1, server2) != 0)
		{
			strcpy_s(szDataServIP, server1);
			dwDataServPort = Settings::getInstance()->getPort();
	...
	if (smWsockServer && smWsockDataServer && !smWsockUserServer)
	{
		...
		if (server3[0] && lstrcmpi(server3, server2) != 0)
		{
			...
			dwUserServPort = Settings::getInstance()->getPort();
	...
	if (smWsockServer && smWsockDataServer && !smWsockExtendServer)
	{
		...
		if (server2[0] && lstrcmpi(server2, server2) != 0)
		{
			...
			dwExtendServPort = Settings::getInstance()->getPort();
...
smWINSOCK *ConnectServer_InfoMain()
{
	if (!smWsockDataServer)
	{
		...
		strcpy_s(server1, sizeof(server1), Settings::getInstance()->getServer1().c_str());
...
smWINSOCK *ConnectServer_GameMain(char *szIP1, DWORD dwPort1, char *szIP2, DWORD dwPort2)
{
	...
	Settings::getInstance()->setServer2(szIP1);
	Settings::getInstance()->setServer3(szIP2);
...
smWINSOCK *ConnectServer_GameMain(char *szIP1, DWORD dwPort1, char *szIP2, DWORD dwPort2, char *szIP3, DWORD dwPort3)
{
	...
	Settings::getInstance()->setServer2(szIP3);
...
//#endif
	...
	case OpCode::OPCODE_CONNECTED:
		...
		if (pData->smMySock == smWsockServer)
		{
			...
			SendNetworkQuality(Settings::getInstance()->getNetworkQuality());
	...
	case OpCode::OPCODE_FAILRECORDDATA:
		...
		{
			Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 11");
	...
	break;
	case PACKET_SEND_VIP:
	{
		PACKET_VIP * packet = (PACKET_VIP*)pData->Buff;
		if (packet->param)
		{

			if (!MatVIP)
			{
				MatVIP = CreateTextureMaterial("image\\Sinimage\\skill\\keep\\VIP.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
				ReadTextures();
			}

			if (cSkill.SearchContiueSkillCODE(PLAYER_VIP) != PLAYER_VIP)
			{
				sSKILL TempSkill;
				memset(&TempSkill, 0, sizeof(sSKILL));
				TempSkill.CODE = PLAYER_VIP;
				TempSkill.UseTime = 604800;
				TempSkill.Point = 1;
				TempSkill.MatIcon = MatVIP;
				sinContinueSkillSet(&TempSkill);
				sinPlaySound(SIN_SOUND_EAT_POTION2);
			}
			memcpy(&IsVIP, packet, sizeof(PACKET_VIP));
		}
	}
	break;
	...
	case OpCode::OPCODE_CLOSECLIENT:
		...
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 13");
		DisconnectFlag = GetCurrentTime();
		break;
	...
	case OpCode::OPCODE_UPDATE_PREMIUMITEM_TIME:
		...
		VIPTime = lpTransPremiumTime->VIP;

		if (chaPremiumitem.m_ThirdEyesTime > PRIME_ITEM_TIME_12H || chaPremiumitem.m_ExpUpPotionTime > PRIME_ITEM_TIME_12H ||
			chaPremiumitem.m_VampiricCuspidTime > PRIME_ITEM_TIME_12H || chaPremiumitem.m_ManaRechargingPTime > PRIME_ITEM_TIME_12H ||
			chaPremiumitem.m_ManaReducePotiontime > PRIME_ITEM_TIME_12H || chaPremiumitem.m_MightofAwellTime > PRIME_ITEM_TIME_12H || chaPremiumitem.m_PhenixPetTime > PRIME_ITEM_TIME_12H ||
			chaPremiumitem.m_HelpPetTime > PRIME_ITEM_TIME_24H || chaPremiumitem.m_StaminaReducePotiontime > PRIME_ITEM_TIME_12H || chaPremiumitem.m_VampiricCuspidEXTime > PRIME_ITEM_TIME_12H)
		{
			break;
		}
	...
	else
	{
		...
		if (szMessage[0] == '/')
		{
			...
			if (IsVIP.param)
			{
				...
				if (len)
				{
					AddChatBuff("> /Skill -> Abre Mestre das Habilidades", 3);
					AddChatBuff("> /Mapa -> Teleport para o mapa", 3);
					AddChatBuff("> /Mix -> Abre o Mix", 3);
					AddChatBuff("> /Force -> Abre o Mestre das Forças", 3);
					AddChatBuff("> /Aging -> Abre o Aging", 3);
					AddChatBuff("> /ItemShop -> Abre o ItemShop", 3);
					AddChatBuff("> /TItemShop -> Abre o ItemShop Viciados", 3);
				}
				...
				len = rsCompString("/Skill", szMessage);
				...
				if (len)
				{
					...
					packet.code = OpCode::OPCODE_SKILL_MENU;
					if (smWsockUserServer)
				...
				if (len)
				{
					cCraftItem.OpenFlag = true;
				}

				len = rsCompString("/Aging", szMessage);

				if (len)
				{
					cAging.OpenFlag = true;
				}

				len = rsCompString("/ItemShop", szMessage);
				...
				if (len)
				{
					...
					CopyMemory(&packet.item, &selectitem, sizeof(SHOP_ITEM));
					packet.size = sizeof(SHOP_BUY);
					packet.code = smTRANSCODE_OPEN_SHOP;
					if (smWsockUserServer)
						smWsockUserServer->Send((char *)&packet, packet.size, TRUE);
				}

				len = rsCompString("/TItemShop", szMessage);
				...
				if (len)
				{
					...
					CopyMemory(&packet.item, &selectitem, sizeof(SHOP_ITEM));
					packet.size = sizeof(SHOP_BUY);
					packet.code = smTRANSCODE_OPEN_TSHOP;
					if (smWsockUserServer)
						smWsockUserServer->Send((char *)&packet, packet.size, TRUE);
				}
			}
	...
	for (cnt = start; cnt < end; cnt++)
	{
		if (cnt >= 0)
		{
			...
			if (ChatBuff[chatCnt].dwIP == dwMyIP)
				SetFontTextColor(hdc, RGB(255, 128, 96));
			else
			{
				SetFontTextColor(hdc, RGB(255, 255, 128));
				...
				case 1:
					...
					break;
				case 2:
					SetFontTextColor(hdc, RGB(96, 256, 255));
					break;
				}
			}
...
int SetIpColor(HDC hdc, DWORD dwIP)
{
	...
	switch (dwIP)
	{
	case 0:		SetFontTextColor(hdc, RGB(255, 128, 96));		break;
	case 1:		SetFontTextColor(hdc, RGB(150, 190, 255));		break;
	case 2:		SetFontTextColor(hdc, RGB(150, 190, 255));		break;
	case 3:		SetFontTextColor(hdc, RGB(230, 160, 255));		break;
	case 4:		SetFontTextColor(hdc, RGB(200, 255, 0));		break;
	case 5:		SetFontTextColor(hdc, RGB(255, 255, 80));		break;
	case 6:		SetFontTextColor(hdc, RGB(200, 255, 255));		break;
	case 9:		SetFontTextColor(hdc, RGB(200, 128, 128));		break;
	case 10:	SetFontTextColor(hdc, RGB(255, 180, 0));		break;
	case 11:	SetFontTextColor(hdc, RGB(112, 219, 219));		break;
	default:	SetFontTextColor(hdc, RGB(255, 255, 192));		break;
	}
		...
		if (cnt >= 0)
		{
			...
			if (len > StrMax)
			{
				...
				if (bsemi)
				{
					...
					SetFontTextColor(hdc, RGB(100, 255, 50));
					dsTextLineOut(hdc, x, y, presemi, prelen);
					GetFontTextExtentPoint(hdc, presemi, prelen, &size);
...
int DisconnectPlayer(smWINSOCK *lpsmSock)
{
	...
	if (smWsockServer && lpsmSock->sock == smWsockServer->sock)
	{
		...
		smWsockServer = 0;
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 14");
	...
	if (smWsockDataServer && lpsmSock->sock == smWsockDataServer->sock)
	{
		...
		smWsockDataServer = 0;
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 15");
		DisconnectFlag = dwTime;
	...
	if (smWsockUserServer && lpsmSock->sock == smWsockUserServer->sock)
	{
		AddChatBuff("Connection lost to Server");
		smWsockUserServer = 0;
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 16");
		DisconnectFlag = dwTime;
	...
	if (smWsockExtendServer && lpsmSock->sock == smWsockExtendServer->sock)
	{
		AddChatBuff("Connection lost to Server");
		smWsockExtendServer = 0;
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 18");
		DisconnectFlag = dwTime;
...
int NetWorkPlay()
{
	...
	if (NetWorkInitFlag == 0)
	{
		...
		switch (Settings::getInstance()->getNetworkQuality())
		{
		case 0:
			NetScoopTime = 3000;
			break;
	...
	else
	{
		...
		if (dwLoadingTime)
		{
			...
			if (dwTime > (dwLoadingTime + NET_LOADING_WAIT_LIMIT))
			{
				...
				Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 19");
				DisconnectFlag = GetCurrentTime();
				dwLoadingTime = 0;
			}
		}
		...
		if (!TransThrowItem.Item.SpecialItemFlag[0])
		{
				...
				(dwItemCodeMask == sinBI2 && TransThrowItem.Item.PotionCount == 300))
			{
				...
				if (dwItemCodeMask == sinBI1 || dwItemCodeMask == sinBI2)
				{
					...
					{
						chaPremiumitem.SetThirdEyesTime(PRIME_ITEM_TIME_6H);
					}
					else if (TransThrowItem.Item.CODE == (sinBI1 | sin13))
					{
						chaPremiumitem.SetExpUpPotionTime(PRIME_ITEM_TIME_6H);
					}
					else if (TransThrowItem.Item.CODE == (sinBI1 | sin14))
					{
						chaPremiumitem.SetThirdEyesTime(PRIME_ITEM_TIME_12H);
					}
					...
					{
						chaPremiumitem.SetManaReducePotionTime(PRIME_ITEM_TIME_6H);
						chaPremiumitem.SetManaReducePotionValue(30);
					}
					else if (TransThrowItem.Item.CODE == (sinBI1 | sin29))
					{
						chaPremiumitem.SetManaReducePotionTime(PRIME_ITEM_TIME_12H);
						chaPremiumitem.SetManaReducePotionValue(30);
					}
					...
					{
						chaPremiumitem.SetPhenixPetTime(PRIME_ITEM_TIME_6H);
					...
					{
						chaPremiumitem.SetStaminaReducePotionTime(PRIME_ITEM_TIME_6H);
						chaPremiumitem.SetStaminaReducePotionValue(30);
					}
					else if (TransThrowItem.Item.CODE == (sinBI1 | sin92))
					{
						chaPremiumitem.SetStaminaReducePotionTime(PRIME_ITEM_TIME_12H);
						chaPremiumitem.SetStaminaReducePotionValue(30);
					}
...
int	TransCheckNetwork()
{
		...
		!smWsockServer || smWsockServer->WaitReadMessage < 0)
	{
		...
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 20");
		DisconnectFlag = GetCurrentTime();
		return FALSE;
	}
	...
	if (smWsockDataServer && smWsockServer != smWsockDataServer)
	{
		...
		{
			Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 21");
			DisconnectFlag = GetCurrentTime();
	...
	if (smWsockUserServer && smWsockServer != smWsockUserServer)
	{
			...
			smWsockUserServer->WaitReadMessage < 0)
		{
			...
			Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 22");
			DisconnectFlag = GetCurrentTime();
			return FALSE;
		}
	...
	if (smWsockExtendServer && smWsockServer != smWsockExtendServer)
	{
			...
			smWsockExtendServer->WaitReadMessage < 0)
		{
			...
			DisconnectServerCode = 3;
			Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 23");
			DisconnectFlag = GetCurrentTime();
			return FALSE;
		}
...
int SendSetHackUser(int StopGame)
{
	...
	if (WarningHack)
	{
		...
		Log::getInstance()->output(LOG_DEBUG, "SetDisconnectFlag: 25");
		DisconnectFlag = GetCurrentTime();
	}
...
int DeleteEventItem_TimeOut(sITEMINFO	*lpsItem)
{
	...
	if (lpsItem->CODE == (sinDA1 | sin32) || lpsItem->CODE == (sinDA1 | sin34) ||
		...
		lpsItem->CODE == (sinDA1 | sin54) || lpsItem->CODE == (sinDA1 | sin55) ||
