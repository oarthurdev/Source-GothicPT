...
int InitPat3D()
{
	...
	MatEachBox = CreateTextureMaterial("image\\cw.png", 0, TRUE, FALSE, FALSE, SMMAT_BLEND_ALPHA);
...
int PlayPat3D()
{
	...
	if ((PlayCounter & 0x3) == 0)
	{
		...
		if ((PlayCounter & 0xFF) == 0)
		{
			...
			if (cSettings::GetInstance()->cWeather)
			{
...
int DrawPatShadow(int x, int y, int z, int ax, int ay, int az)
{
	...
	if (!cSettings::GetInstance()->cShadows)
		return TRUE;
	...
	if (chrEachPlayer.Flag && chrEachMaster && chrEachMaster->Flag)
	{
		...
		else
		{
			...
			if (SizeMode)
			{
				...
				sizecheck.cx = 0;
				sizecheck.cy = 0;
				GetFontTextExtentPoint( chrEachPlayer.smCharInfo.szName, lstrlen(chrEachPlayer.smCharInfo.szName), &sizecheck);
				dist = sizecheck.cx >> 1;

				SetFontTextColor( RGB(0, 0, 0));
				dsTextLineOut( MidX - dist + 1, MidY - (h >> 1) + h - 33, chrEachPlayer.smCharInfo.szName, len);
				SetFontTextColor( RGB(255, 255, 180));
				dsTextLineOut( MidX - dist, MidY - (h >> 1) + h - 32, chrEachPlayer.smCharInfo.szName, len);
			...
			else
			{
				...
				SetFontTextColor( RGB(0, 0, 0));
				dsTextLineOut( MidX - dist + 1, MidY - (h >> 1) + h - 17, chrEachPlayer.smCharInfo.szName, len);
				...
				dsTextLineOut( MidX - dist, MidY - (h >> 1) + h - 16, chrEachPlayer.smCharInfo.szName, len);

				if (ClanLen)
				{
					SetFontTextColor( RGB(0, 0, 0));
					dsTextLineOut( ClanX + 1, ClanY + 1, ClanInfo[chrEachPlayer.ClanInfoNum].ClanInfoHeader.ClanName, ClanLen);
					SetFontTextColor( RGB(150, 255, 200));
					dsTextLineOut( ClanX, ClanY, ClanInfo[chrEachPlayer.ClanInfoNum].ClanInfoHeader.ClanName, ClanLen);
				}
				...
				if (smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR && cInterFace.sInterFlags.MapOnFlag)
				{
					...
					dsTextLineOut( MidX - 50, ly, szBuff, lstrlen(szBuff));
					ly += 16;
					...
					if (DisplayDebug)
					{
						...
						dsTextLineOut( MidX - 100, ly, szBuff, lstrlen(szBuff));
						ly += 16;
