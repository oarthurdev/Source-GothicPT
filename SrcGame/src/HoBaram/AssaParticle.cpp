...
void cASSAPARTSHOT::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	...
	else if(EffectType == ASSA_SHOT_LASER || EffectType == ASSA_SHOT_LASER2 || EffectType == ASSA_SHOT_LASER3 || EffectType == SKILL_JUDGE)
	{
...
void cASSAPARTSHOT::Start(POINT3D *pCurPosi, POINT3D *pDestPosi, int type)
{
	...
	TimeCount = 0;
	EffectType = type;
	if(EffectType == ASSA_SHOT_LASER || EffectType == ASSA_SHOT_LASER2 || EffectType == ASSA_SHOT_LASER3 || EffectType == SKILL_JUDGE)
	{
		Face.r = 200;
		Face.g = 200;
		Face.b = 200;
		...
		else if (EffectType == SKILL_JUDGE)

		{
			matNum = (int *)AssaSearchRes("Ray04.bmp", SMMAT_BLEND_LAMP);
		}
...
CShootingSkillEffect::CShootingSkillEffect()
{
...
void CShootingSkillEffect::Start(POINT3D *pCur, smCHAR *destChar, D3DXVECTOR3 *velocity, int delay, char *pEffectName1, char *pEffectName2, unsigned int SkillCode)
{
	Pos.x = pCur->x;
	Pos.y = pCur->y;
	Pos.z = pCur->z;
	...
	if (pEffectName1)
	{
		ParticleID = g_NewParticleMgr.Start(pEffectName1, Pos);
	}

	if (pEffectName2)
	{
		ParticleIDExt1 = g_NewParticleMgr.Start(pEffectName2, Pos);
	}

	Max_Time = 2000;
	Time = -delay;
	dwSkillCode = SkillCode;
}

void CShootingSkillEffect::Main()
{
	...
	if (Time >= 0)
	{
		if (ParticleID != -1)
		{
			...
			if (length < 15 || Time > 100)
			{
				...
				{
				case SKILL_DARKBOLT:
					AssaParticle_AttackSkillEffect("DarkBoltHit", DestChar, 0);
					break;
				case SKILL_DARKWAVE:
					AssaParticle_AttackSkillEffect("DarkWaveHit", DestChar, 0);
					break;
				}
			}
			...
			if (length < 100.0)
			{
				Velocity.x = term.x * 0.85f;
				Velocity.y = term.y * 0.85f;
				Velocity.z = term.z * 0.85f;
			}
			...
			Pos.x = Velocity.x * fONE + Pos.x;
			Pos.y = Velocity.y * fONE + Pos.y;
			Pos.z = Velocity.z * fONE + Pos.z;
...
CKeepSkillEffect::CKeepSkillEffect()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void CKeepSkillEffect::Start(char *FileName, smCHAR *character, int liveCount, unsigned int dwSkillCode)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	Posi.y = pChar->pY;
	Posi.z = pChar->pZ;
	ParticleID = g_NewParticleMgr.Start(FileName, Posi);

	Max_Time = liveCount * 70 * (liveCount + 2);
	CODE = dwSkillCode;
	EndFlag = FALSE;
}
void CKeepSkillEffect::Main()
{
...
void AssaClassUpWeapon::Main()
{
	...
	if(pChar->HvRightHand.dwItemCode != 0)
	{
		if (pChar->smCharInfo.JOB_CODE == 9 && StageField[pChar->OnStageField]->State == FIELD_STATE_VILLAGE)
			mWorld = &pChar->WaistObjBip[0]->mWorld;
		else
	...
	{
		if((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2) == 
			sinWS1)
		{
