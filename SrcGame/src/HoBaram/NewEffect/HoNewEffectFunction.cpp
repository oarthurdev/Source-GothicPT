...
void SkillCelestialMusPel(smCHAR* pChar, float time, float attackTime, int dwSkillCode)
{
	...
	effectType->Init(pChar, time, attackTime, dwSkillCode);
...
{
	HoEffectGroup *effectGroup = MainWindow.LoadScript("Effect\\NewEffect\\PVP.lua");
	if (effectGroup)
	{
		HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
		effectType->Init(pChar, effectGroup, 0, 0.f);
		POINT3D pos;
		pos.x = pChar->pX;
		pos.y = pChar->pY;
		pos.z = pChar->pZ;
...
void EffectGM(smCHAR *pChar)
{
	HoEffectGroup *effectGroup = MainWindow.LoadScript("Effect\\NewEffect\\Gm.lua");
	if (effectGroup)
	{
		HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
		effectType->Init(pChar, effectGroup, 0, 0.f);
		...
		int num = g_NewParticleMgr.Start("GameMaster", pos);
		effectType->SetParticleID(num);
		HoEffectManager::GetInstance()->AddEffect(effectType);
	}
}
...
{
	HoEffectGroup *effectGroup = MainWindow.LoadScript("Effect\\NewEffect\\TopPVPClass.Lua");
	if (effectGroup)
	{
		HoEffectType_Attach_Avater *effectType = new HoEffectType_Attach_Avater();
		effectType->Init(pChar, effectGroup, 0, 0.f);
		...
		int num = g_NewParticleMgr.Start("TopLevel", pos);
		effectType->SetParticleID(num);
		HoEffectManager::GetInstance()->AddEffect(effectType);
	}
}
