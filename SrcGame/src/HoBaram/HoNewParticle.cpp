...
void HoNewParticleEvent::ProcessPropEqualsValue(HoMinMax<D3DXVECTOR3> &prop,
												std::vector<HoNewParticleEmitterToken>::iterator &TokenIter,
												std::vector<HoNewParticleEmitterToken>::iterator &EndIter)
{
...
void HoNewParticleEvent_PartAngle::DoItToIt(HoNewParticle &part)
{
	...
	if(NextFadeEvent)
	{
		D3DXVECTOR3 newAngle = static_cast<HoNewParticleEvent_PartAngle *>(NextFadeEvent)->GetPartAngle().GetRandomNumInRange();
...
void HoNewParticleEvent_LocalAngle::DoItToIt(HoNewParticle &part)
{
	...
	if(NextFadeEvent)
	{
		D3DXVECTOR3 newAngle = static_cast<HoNewParticleEvent_LocalAngle *>(NextFadeEvent)->GetLocalAngle().GetRandomNumInRange();
		float timedelta = NextFadeEvent->GetActualTime() - GetActualTime();
		if(timedelta == 0) timedelta = 1;
...
void HoNewParticleEvent_Velocity::DoItToIt(HoNewParticle &part)
{
	if(!IsFade())
	{
		...
		else if(Flag == 1)
		{
			...
			D3DXVECTOR3 newvalue = Velocity.GetRandomNumInRange();
	...
	if(NextFadeEvent)
	{
		D3DXVECTOR3 newvalue = static_cast<HoNewParticleEvent_Velocity *>(NextFadeEvent)->GetVelocity().GetRandomNumInRange();
...
bool HoNewParticleEmitterTokenizer::ProcessVector(HoMinMax<D3DXVECTOR3> &v,
												  std::vector<HoNewParticleEmitterToken>::iterator &TokenIter,
												  std::vector<HoNewParticleEmitterToken>::iterator &EndIter)
{
...
void HoNewParticleEmitter::init()
{
	...
	SetPosRange(HoMinMax<D3DXVECTOR3>(temp0, temp1));
	Stop();
	FadeStopFlag = false;
	AttachPosFlag = false;
	FastStopFlag = false;
...
void HoNewParticleEventSequence::Reset()
{
	...
	Gravity = HoMinMax<D3DXVECTOR3>(temp0, temp0);
	EmitRadius = HoMinMax<D3DXVECTOR3>(temp1, temp1);
	for(std::vector<HoNewParticleEvent *>::iterator i = Events.begin(); i != Events.end(); i++)
	{
...
smRENDFACE *HoNewParticleEventSequence::AddFace2dPlane(smFACE2D *face, D3DXVECTOR3 inAngle)
{
	int x, y, z;
	int width, height;
...
bool HoNewParticleEventSequence::AddFaceThree(smFACE2D *face, D3DXVECTOR3 inAngle)
{
	int x, y, z;
	int width, height;
...
smRENDFACE *HoNewParticleEventSequence::AddFace2DBillBoard(smFACE2D *face, D3DXVECTOR3 inAngle)
{
	int x, y, z;
	int rx, ry, rz;
	int width, height;
