...
int HoSky::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	if(smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
	{
		...
		if(ColorInfoFlag)
		{
			...
			dsTextLineOut( 11, 101, buffer, strlen(buffer));

			SetFontTextColor( RGB(255, 255, 255));
			dsTextLineOut( 10, 100, buffer, strlen(buffer));
			...
			SetFontTextColor( RGB(0, 0, 0));
			dsTextLineOut( 11, 116, buffer, strlen(buffer));

			SetFontTextColor( RGB(255, 255, 255));
			dsTextLineOut( 10, 115, buffer, strlen(buffer));
...
int HoSky::Main()
{
	   ...
	   (FogEndPos1 != FogEndPos2))
	{
		...
		if((smRender.m_dwFogColor % 10) == 0)
			d3ddev->SetRenderState(D3DRS_FOGCOLOR, smRender.m_dwFogColor);

		smRender.m_fFogStNum = (float)FogStartPos1*4.f;
		if((FogStartPos1 % 10) == 0)
			d3ddev->SetRenderState(D3DRS_FOGSTART, *((DWORD*)(&smRender.m_fFogStNum)));

		if((FogEndPos1 % 10) == 0)
			smRender.m_fFogEndNum = (float)FogEndPos1*4.f;
		d3ddev->SetRenderState(D3DRS_FOGEND, *((DWORD*)(&smRender.m_fFogEndNum)));
...
void HoSky::Create()
{
	...
	if(cSettings::GetInstance()->cBPP == 32)
	{
