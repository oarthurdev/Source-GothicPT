...
struct	sDEF_ITEMINFO
{
	...
	sDEF_ITEMINFO()
	{
		...
		ZeroMemory(&sDurability, sizeof(sDurability));
		ZeroMemory(&sResistance, sizeof(sResistance));
		ZeroMemory(&sDamage, sizeof(sDamage));
		ZeroMemory(&sAttack_Rating, sizeof(sAttack_Rating));
		ZeroMemory(&sDefence, sizeof(sDefence));
		ZeroMemory(&fBlock_Rating, sizeof(fBlock_Rating));
		ZeroMemory(&fAbsorb, sizeof(fAbsorb));
		ZeroMemory(&fSpeed, sizeof(fSpeed));
		ZeroMemory(&Increase_Life, sizeof(Increase_Life));
		ZeroMemory(&Increase_Mana, sizeof(Increase_Mana));
		ZeroMemory(&Increase_Stamina, sizeof(Increase_Stamina));
		ZeroMemory(&fMana_Regen, sizeof(fMana_Regen));
		ZeroMemory(&fLife_Regen, sizeof(fLife_Regen));
		ZeroMemory(&fStamina_Regen, sizeof(fStamina_Regen));
		ZeroMemory(&sSpecial_Defence, sizeof(sSpecial_Defence));
		ZeroMemory(&fSpecial_Absorb, sizeof(fSpecial_Absorb));
		ZeroMemory(&fSpecial_fSpeed, sizeof(fSpecial_fSpeed));
		ZeroMemory(&fSpecial_Magic_Mastery, sizeof(fSpecial_Magic_Mastery));
		ZeroMemory(&fSpecial_Mana_Regen, sizeof(fSpecial_Mana_Regen));
		ZeroMemory(&Lev_Attack_Rating, sizeof(Lev_Attack_Rating));
		ZeroMemory(&dwJobBitCode_Random, sizeof(dwJobBitCode_Random));
		JobBitCodeRandomCount = 0;
		ZeroMemory(&sGenDay, sizeof(sGenDay));
		ZeroMemory(DefCompressData, sizeof(DefCompressData));
		DefCompressDataLen = 0;
