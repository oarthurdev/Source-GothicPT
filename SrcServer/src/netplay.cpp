...
int DisconnectPlayer(smWINSOCK *lpsmSock)
{
	...
	if(smWsockServer && lpsmSock->sock == smWsockServer->sock)
	{
		...
		smWsockServer = 0;
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 14");
	...
	if(smWsockDataServer && lpsmSock->sock == smWsockDataServer->sock)
	{
		...
		smWsockDataServer = 0;
		//LOG(LOG_DEBUG, "SetDisconnectFlag: 15");
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 15");
		DisconnectFlag = dwTime;
	...
	if(smWsockUserServer && lpsmSock->sock == smWsockUserServer->sock)
	{
		AddChatBuff("Connection lost to Server");
		smWsockUserServer = 0;
		//LOG(LOG_DEBUG, "SetDisconnectFlag: 16");
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 16");
		DisconnectFlag = dwTime;
	...
	if(smWsockExtendServer && lpsmSock->sock == smWsockExtendServer->sock)
	{
		AddChatBuff("Connection lost to Server");
		smWsockExtendServer = 0;
		//LOG(LOG_DEBUG, "SetDisconnectFlag: 18");
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 18");
		DisconnectFlag = dwTime;
		...
		if(!TransThrowItem.Item.SpecialItemFlag[0])
		{
				...
				(dwItemCodeMask == sinBI2 && TransThrowItem.Item.PotionCount == 300))
			{
				...
				if (dwItemCodeMask == sinBI1 || dwItemCodeMask == sinBI2)
				{
					...
					else if (TransThrowItem.Item.CODE == (sinBI2 | sin89))
					{
					chaPremiumitem.SetThirdEyesTime(60*60*24*2);
					}
					else if (TransThrowItem.Item.CODE == (sinBI2 | sin90))
					{
					chaPremiumitem.SetThirdEyesTime(60*60*24*3);
					}
...
int	TransCheckNetwork()
{
		...
		!smWsockServer || smWsockServer->WaitReadMessage < 0)
	{
		...
		//LOG(LOG_SERVER, "SetDisconnectFlag: 20");
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 20");
	...
	if(smWsockDataServer && smWsockServer != smWsockDataServer)
	{
		...
		{
			//LOG(LOG_DEBUG, "SetDisconnectFlag: 21");
			Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 21");
			DisconnectFlag = GetCurrentTime();
	...
	if(smWsockUserServer && smWsockServer != smWsockUserServer)
	{
			...
			smWsockUserServer->WaitReadMessage < 0)
		{
			...
			//LOG(LOG_DEBUG, "SetDisconnectFlag: 22");
			Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 22");
			DisconnectFlag = GetCurrentTime();
			return FALSE;
		}
	...
	if(smWsockExtendServer && smWsockServer != smWsockExtendServer)
	{
			...
			smWsockExtendServer->WaitReadMessage < 0)
		{
			...
			DisconnectServerCode = 3;
			//LOG(LOG_DEBUG, "SetDisconnectFlag: 23");
			Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 23");
			DisconnectFlag = GetCurrentTime();
			return FALSE;
		}
...
int SendSetHackUser(int StopGame)
{
	...
	if(WarningHack)
	{
		...
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 25");
		DisconnectFlag = GetCurrentTime();
	}
...
int XTrap_Recv(smTRANS_COMMAND *lpPacket, smWINSOCK *lpsmSock)
{
	...
	case smXTRAP_COMMAND_SECURITY:
		...
		Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 30");
		DisconnectFlag = GetCurrentTime();
		break;
...
int XTrap_Recv(smTRANS_COMMAND *lpPacket, smWINSOCK *lpsmSock)
{
	...
	if(lpPacket->WParam == XTRAP_CMD_SEEDKEY)
	{
		...
		else
		{
			...
			Log::getInstance()->output(LOG_SERVER, "SetDisconnectFlag: 31");
			DisconnectFlag = GetCurrentTime();
		}
