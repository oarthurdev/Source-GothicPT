...
int HoEtc2dPrimitive::StartUp(int x, int y)
{
	...
	for(int count = 0; count < 4; count++)
	{
		...
		TLVertex[count].color = RGBA_MAKE(255, 255, 255, 255);
		TLVertex[count].specular = RGBA_MAKE(0, 0, 0, 0);
		TLVertex[count].sz = 0;
	}
...
int HoEtc2dPrimitive::Draw()
{
	if(StartFlag)
	{
		...
		if(FAILED(renderDevice.SetTexture(0, smMaterial[mat].smAnimTexture[AnimationData.SequenceDataBuffer[AniDataIndex].AnimFrame[CurrentFrame].ImageNum]->lpD3DTexture)))
		{
			OutputDebugString("SetTexture Error");
			return FALSE;
		}

		DRZD3DTLVERTEX tlVertex[MAX_2DVERTEX];
		memcpy(tlVertex, TLVertex, sizeof(DRZD3DTLVERTEX)*VertexCount);
		...
		renderDevice.SetTextureStageState(0, D3DTSS_MINFILTER, D3DFILTER_LINEAR);
		renderDevice.SetTextureStageState(0, D3DTSS_MAGFILTER, D3DFILTER_LINEAR);
...
int HoEffectMgr::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	renderDevice.SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, TRUE);
	renderDevice.SetRenderState(D3DRENDERSTATE_LASTPIXEL, FALSE);
	return TRUE;
}
