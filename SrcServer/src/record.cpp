...
int rsRECORD_DBASE::ResotrRecordData(smCHAR_INFO *lpCharInfo, sITEM *lpItems, sITEM *lpItems2, sITEM *lpMouseItem)
{
	...
	if (TransRecordData.GameSaveInfo.wForceOrbUsing[0] && TransRecordData.GameSaveInfo.wForceOrbUsing[1])
	{
		...
		if (TransRecordData.GameSaveInfo.wForceOrbUsing[0] >= sin01 && TransRecordData.GameSaveInfo.wForceOrbUsing[0] <= sin12)
		{
...
int rsRECORD_DBASE::SendRecordDataToClient(rsPLAYINFO *lpPlayInfo, char *szName, smWINSOCK *lpsmSock, int Mode)
{
	...
	if (lpPlayInfo->getSecurity() > AccountTypes::SEC_VIP && lpPlayInfo->sLowLevel_PresentItem[0] == 0)
...
int rsRECORD_DBASE::RecvRecordDataFromClient(TRANS_RECORD_DATAS *lpTransRecord, rsPLAYINFO *lpPlayInfo)
{
	...
	if (lpTransRecord->Count >= lpTransRecord->Total - 1 && (TransRecordData.code == OpCode::OPCODE_RECORDDATA || TransRecordData.code == 5004))
	{
		...
		lstrcpy(TransRecordData.GameSaveInfo.szMasterID, lpPlayInfo->getAccount().c_str());
...
int rsLoadServerForce(rsPLAYINFO *lpPlayInfo, sGAME_SAVE_INFO *lpGameSaveInfo)
{
	...
	if (lpGameSaveInfo->BattleExp > 0)
		lpPlayInfo->BattleExp = lpGameSaveInfo->BattleExp;
	else
		lpPlayInfo->BattleExp = 0;

	lpPlayInfo->BattleLevel = GetBattleLevel(lpPlayInfo->BattleExp);


	packet.size = sizeof(smTRANS_COMMAND);
	packet.code = smTRANSCODE_UPDATE_BATTLE_LEVEL;
	packet.LParam = lpPlayInfo->BattleLevel;
	packet.WParam = lpPlayInfo->BattleExp;
	packet.SParam = GetExpNextLevel(lpPlayInfo->BattleLevel);
	if (lpPlayInfo->lpsmSock)
	...
	if (lpGameSaveInfo->QuestDiary_State > 0)
		...
		lpPlayInfo->QuestDiary_State = 0;

	if (lpGameSaveInfo->QuestDiary_Kill > 0)
		lpPlayInfo->QuestDiary_Kill = lpGameSaveInfo->QuestDiary_Kill;
	else
		lpPlayInfo->QuestDiary_Kill = 0;

	packet.size = sizeof(smTRANS_COMMAND);
	packet.code = smTRANSCODE_UPDATE_DIARY;
	packet.LParam = lpPlayInfo->QuestDiary_State;
	packet.WParam = lpPlayInfo->QuestDiary_Kill;
	...
	packetc.size = sizeof(PACKET_CROWN);
	packetc.code = OpCode::BLESS_CROWN;
	packetc.Silver = GetClanCode(rsBlessCastle.ClanTop10.dwUserCode[1]);
	packetc.Bronze = GetClanCode(rsBlessCastle.ClanTop10.dwUserCode[2]);
...
//Exp de Batalha
	if (lpPlayInfo->BattleExp)
		lpGameSaveInfo->BattleExp = lpPlayInfo->BattleExp;
	else
...
//Desafio Diario
	...
	if (lpPlayInfo->QuestDiary_State)
		...
		lpGameSaveInfo->QuestDiary_State = 0;

	if (lpPlayInfo->QuestDiary_Kill)
