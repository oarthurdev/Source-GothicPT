...
int rsPLAYINFO::UpdatePartyMode(rsPLAYINFO* lpPlayInfo, int Mode)
{
	if (lpPartyMaster != this) return FALSE;

	TRANS_CHATMESSAGE TransChatMessage;
	TransChatMessage.code = OpCode::OPCODE_WHISPERMESSAGE;
	wsprintf(TransChatMessage.szMessage, "> O grupo foi alterado para o modo %s!", Mode == 2 ? "Hunt" : "Exp");
	TransChatMessage.size = lstrlen(TransChatMessage.szMessage) + 32;
	TransChatMessage.dwIP = 0;
	TransChatMessage.dwObjectSerial = 0;

	smTRANS_COMMAND smTransCommand;
	smTransCommand.code = OpCode::OPCODE_UPDATE_PARTYMODE;
	...
	for (int cnt = 0; cnt < PARTY_PLAYER_MAX; cnt++)
	{
		if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt]->lpsmSock)
		{
			lpPartyPlayers[cnt]->iPartyMode = Mode;
			lpPartyPlayers[cnt]->lpsmSock->Send((char*)&TransChatMessage, TransChatMessage.size, TRUE);
...
int	rsPLAYINFO::JoinPartyPlayer(rsPLAYINFO* lpPlayInfo)
{
	...
	for (cnt = 0; cnt < PARTY_PLAYER_MAX; cnt++)
	{
		if (!lpPartyPlayers[cnt])
		{
			...
			lpPlayInfo->dwPartyInfo = PARTY_STATE_MEMBER;
			lpPlayInfo->lpPartyMaster = this;
			lpPlayInfo->iPartyMode = iPartyMode;
			UpdatePartyPlayer();
			return TRUE;
		}
	}
...
int	rsPLAYINFO::DeletePartyPlayer(rsPLAYINFO* lpPlayInfo)
{
	...
	for (cnt = 0; cnt < PARTY_PLAYER_MAX; cnt++)
	{
		if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt] == lpPlayInfo)
		{
			...
			lpPartyPlayers[cnt]->iPartyMode = 0;
			lpPartyPlayers[cnt] = 0;
...
int	rsPLAYINFO::UpdatePartyPlayer()
{
	...
	TransPartyUsers.iPartyMode = lpPartyMaster->iPartyMode;
	...
	TransPartyUsers.PartyUser[cnt2].Mana[0] = lpPartyMaster->smCharInfo.Mana[0];
	TransPartyUsers.PartyUser[cnt2].Mana[1] = lpPartyMaster->smCharInfo.Mana[1];
	...
	TransPartyUsers.PartyUser[cnt2].JobCode = lpPartyMaster->smCharInfo.JOB_CODE;
	...
	for (cnt = 0; cnt < PARTY_PLAYER_MAX; cnt++)
	{
		if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt] != lpPartyMaster)
		{
			...
			TransPartyUsers.PartyUser[cnt2].Mana[0] = lpPartyPlayers[cnt]->smCharInfo.Mana[0];
			TransPartyUsers.PartyUser[cnt2].Mana[1] = lpPartyPlayers[cnt]->smCharInfo.Mana[1];
			...
			TransPartyUsers.PartyUser[cnt2].JobCode = lpPartyPlayers[cnt]->smCharInfo.JOB_CODE;
...
int	rsPLAYINFO::UpdatePartyPlayInfo()
{
	...
	TransPartyPlayInfo.PlayInfo[cnt2].Mana[0] = smCharInfo.Mana[0];
	TransPartyPlayInfo.PlayInfo[cnt2].Mana[1] = smCharInfo.Mana[1];
...
// PORCENTAGEM DE EXP POR PLAYERS NA PARTY
...
extern bool CanReceiveExp(int Level, int Field);
...
int rsPLAYINFO::GetPartyExp(smCHAR* lpChar, rsPLAYINFO* lpPlayInfo)
{
	...
	cQuestHandler.KillMonster(lpPlayInfo, lpChar, false);
	...
	if (Exp)
	{
		...
		for (int cnt = 0; cnt < PARTY_PLAYER_MAX; cnt++)
		{
			if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt]->lpsmSock)
			{
				...
				//if(!CanReceiveExp(lpPartyPlayers[cnt]->smCharInfo.Level, lpPartyPlayers[cnt]->Position.Area))
				//	smTransCommand.WParam = 0;
				...
				if (smTransCommand.WParam)
				{
					...
					if (lpPartyPlayers[cnt]->tVIP > 0)
					{
						ExpUp += VIPExpAdd;
					}

					if (lpPartyPlayers[cnt]->iPartyMode == 1)
					{
						File server("config.ini");
						ExpUp += server.readInt("Rate", "ExpParty");
					}
